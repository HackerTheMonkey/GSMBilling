...
import java.io.*;
public class TrunkOutFormatConverter
{
		...
		throws Exception
	{
		...
		for (int i=0;i<dirList01.length;i++)
		{
			...
			FileWriter outputFile = new FileWriter("d:\\trunkout_" + i + ".txt");
			for (int j=0;j<dirList02.length;j++)
			{
				...
				while (buffer01.read() != -1)
				{
					...
					if (myLine.substring(237,245).trim().equals("100"))
					{
						//outputFile.write(myLine + '\n');
						String cdrType = myLine.substring(5,9).trim();
						String caller = myLine.substring(10,30).trim();
...
//						outputFile.write("3" + generateSpace(5 - 1));
